    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float wave = cos(i.uv.x*TAU*5+_Time.y*10)*0.01;    
                float trigonometry = cos((i.uv.y+wave-_Time.y*0.3)*TAU*_LineNumber)*0.5+0.5;
                ...
                float4 result = trigonometry*result02;
                float4 result4 = lerp(_ColorA,_ColorB,i.uv.y);

                return  (result*result4)*(abs(i.normal.y)<0.999);
